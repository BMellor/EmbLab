...
int main(void) {
    ...
    // Reconfigure the SysTick Timer and interrupt
    SysTick_Config(HAL_RCC_GetHCLKFreq()/1000); 
    NVIC_SetPriority(SysTick_IRQn, 3);  // Change priority and allow EXTI interrupt to block SysTick
    
    GPIOC->BSRR = GREEN; // Start with green LED set
    while (1) {
        HAL_Delay(400);
        GPIOC->ODR ^= RED; // Toggle red LED as main loop heartbeat
    } 
}

...
At their core, all timers are simple registers which increment or decrement whenever a trigger signal occurs. Upon this basic functionality is built the capability to generate hardware interrupts, like the SysTick. However, the SysTick is the simplest of the onboard timer peripherals.
    ...
    \subsection{Timers in the STM32F072}
        ...
        Figure \vref{avaliable_timers} shows the timers available in the specific chip used in the labs. When deciding on a timer to use for an application, it is helpful to understand their capabilities and if they are suitable for the task. A brief breakdown of the information in figure \vref{avaliable_timers} is as follows:
...
\section{Using the Timer Documentation}
    ...
    Figure \vref{avaliable_timers} introduced the set of available timers and indicated their characteristics. For the remainder of this lab manual, we will be focusing on the documentation materials for TIM2 \& TIM3. 
...
\section{Using Timers to Generate Interrupts and Events} \label{timer_interrupts}
    ...
    There are three different counting modes that a timer can operate under; up, down, and center-aligned (up/down). Figure \vref{count_modes} shows a graphical representation of these counting modes.  
    ...
    \begin{exercise}[Using Timer Interrupts]
        ...
        \begin{enumerate}
            ...
            \begin{itemize}
                ...
                \begin{itemize}
                    \item See example \vref{example1} and use the prescaler (PSC) register. 
                \end{itemize}
...
%        The capture compare mode register can be changed on-the-fly, moving the event point. This is often used in PWM mode.
    ...
    PWM is used for a variety of purposes. A few common examples are audio, light dimming, and motor speed control. Figure \vref{pwm_sine} shows how an analog sine-wave is approximated by a digital PWM signal. 
...
%    Use chip datasheet (Pinouts and pin descriptions section)
    ...
    Navigate past the package maps until you find Table 13 \textit{STM32F072x8/xB pin definitions}. This table provides pin number to pin name mappings, pin characteristics and limitations, and available alternate functions. Figure \vref{chip_pins} shows an annotated portion of table 13. 
...
%    (marked figure of table 14-19, pick a GPIO and an alternate function) 
    ...
    \begin{example}[Determining an Alternate Function's Number] 
        \label{example3}
Figure \vref{func_pins} shows a portion of Table 15 in the pinouts and pin descriptions section of the device datasheet. This table lists the alternate functions for the GPIOB pins of the device. Continuing from the previous example, we can begin by finding the row representing PB1, the pin chosen with the TIM3\_CH4 function found in Table 13. 
...
\begin{exercise}[Measuring PWM Output]
    In exercise \vref{ex2} you configured channel 1 to \textit{PWM mode 2} and channel 2 to \textit{PWM mode 1}. In this exercise you will be exploring the difference between the two modes and the effect of the CCRx register on the output duty cycle. 
